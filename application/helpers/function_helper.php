...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL)
{
    ...
    if (!empty($product)) {
        ...
        for ($i = 0; $i < count($product); $i++) {
            ...
            $product[$i]['availability'] = isset($product[$i]['availability']) && !empty($product[$i]['availability']) ? $product[$i]['availability'] : '';
...
function get_price_range_of_product($product_id = '')
{
    ...
    $response = $t->db->select('is_prices_inclusive_tax,price,special_price,tax.percentage as tax_percentage')->where("pv.status=1")->get('products p')->result_array();
